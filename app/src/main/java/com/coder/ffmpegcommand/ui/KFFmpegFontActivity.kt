...
import com.coder.ffmpegcommand.databinding.ActivityFfmpegFontBinding
import com.coder.ffmpegcommand.model.CommandBean
import com.coder.ffmpegcommand.ui.adapter.FFmpegCommandAdapter
import com.coder.ffmpegcommand.ui.dialog.PromptDialog
import com.coder.ffmpegcommand.utils.FileIOUtils
import com.coder.ffmpegcommand.utils.FileUtils
import com.coder.ffmpegcommand.utils.ToastUtils
...
class KFFmpegFontActivity : AppCompatActivity() {
    ...
    private fun drawText() {
        ...
        // ffmpeg -y -i input -vf "drawtext=fontfile=${字体路径}/Pingfang.ttf:shadowx=5:shadowy=5:shadowcolor=black:text=FFmpegCommand:x=(w-text_w)/2:y=(h-text_h)/2:fontsize=100:fontcolor=white" -c:v libx264 output
        ...
        val commands = CommandParams()
            ...
            .append("1500k")
            .append("-vf")
            .append("drawtext=fontfile=${localFonts[4]}:shadowx=5:shadowy=5:shadowcolor=DimGray:text=FFmpegCommand:x=(w-text_w)/2:y=(h-text_h)/2:fontsize=100:fontcolor=white")
            .append("-c:v")
            .append("h264_mediacodec")
            .append(outputPath)
            .get()
    ...
    private fun subtitles() {
        ...
        // ffmpeg -y -i input -vf "subtitles=${字体文件位置}:force_style=force_style='fontname=苹方 常规,fontsize=40,PrimaryColour=&H000000,OutlineColour=&HF0F0F0,Italic=0,BorderStyle=1,Alignment=2'" -c:v libx264 output
